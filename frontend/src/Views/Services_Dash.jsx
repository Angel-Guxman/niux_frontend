  ...
  import OptionsServices_Dash from '../Components/Dashboard/Services/OptionsServices_Dash';
  ...
  const Services_Dash = () => {
      ...
      const handleDeleteService = (serviceId) => {
        ...
        .then((willDelete) => {
          if (willDelete) {
            niuxApi.delete(`/services/${serviceId}`) // Ajustar la ruta según tu API
              .then(() => {
                setUsers(users.filter(service => service.id !== serviceId));
                swal("Servicio eliminado con éxito", { icon: "success" });
              })
              .catch(error => {
                console.error('Error al eliminar el servicio', error);
                swal("Error al eliminar el servicio", { icon: "error" });
              });
          }
        });
      };
    ...
    <>
      <div>
        ...
        {
          <div className="">
            <OptionsServices_Dash onDeleteSelected={handleDeleteSelected}/>
              ...
              <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                <thead className="text-xs text-gray-700 uppercase bg-gray-900 dark:bg-gray-700 dark:text-gray-400">
                  <tr>
                    ...
                    <th className="text-white px-6 py-3">DESCRIPCIÓN</th>
                    <th className="text-white px-6 py-3">TAG</th>
                ...
                {users.filter(user => user.isActive).map((user) => (
                    <tr key={user.id} className="bg-white hover:bg-gray-200 border-b dark:bg-gray-800 dark:border-gray-700">
                      ...
                      <td className="">
                        <Link to="/dashboard/update-service" className="flex items-center">
                          <span className="flex items-center justify-center w-25 h-25 bg-purple-100 uppercase text-purple-600 rounded-full font-bold border border-purple-600/30 mr-4">{getInitials(user.title)}</span>
                          {user.title}
                        </Link>
                      </td>
                      ...
                      <td className="px-6 py-4">{user.isActive ? "True":"False"}</td>
                      <td className="px-6 py-4 text-right">
                        <Link to={`/dashboard/edit-service/${user.id}`} className="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                          Editar
                        </Link>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <button  onClick={() => handleDeleteService(user.id)} className="font-medium text-red-600 dark:text-red-500 hover:underline">
                          Eliminar
                        </button>
                      </td>
                    </tr>
